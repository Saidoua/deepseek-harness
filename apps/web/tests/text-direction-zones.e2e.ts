// Keyless assembled-browser coverage for zone-scoped text direction over the
...
// does to the rendered page, which only a real browser resolves: the frame
// keeps its placement, the transcript and composer text flip to the reading
// edge, and a code block keeps column order.
...
describe('web e2e: zone-scoped text direction', () => {
  ...
  it('flips zone text to the reading edge while the frame stays put', async () => {
    ...
    expect(composerBefore.direction).toBe('ltr')

    await setDirection(page, 'rtl')
    const frameAfter = await box(page, '[class*="sidebarCol"]')
    const composerAfter = await box(page, composer)

    // The whole point of scoping: the sidebar column does not move or resize,
    // so no chrome the user navigates by changes place with the language.
    expect(frameAfter).toEqual(frameBefore)
    // The marked zone does flip, and its text starts at the reading edge.
    expect(composerAfter.direction).toBe('rtl')
    expect(composerAfter.textAlign).toBe('start')
    // The zone's own box is laid out by the frame, so it stays where it was.
    ...
    expect((await box(page, composer)).direction).toBe('ltr')
