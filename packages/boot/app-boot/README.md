...
- **Linked directories.** A profile link to an external directory admits its importers to peer-aware ancestor lookup, even without its own `package.json`. At each `D/node_modules` position, current `D/package.json` peer names present in the runtime table use the runtime package; other names use the physical candidate. A nearer physical package precedes a later peer declaration, and a peer position needs no physical `node_modules`. A first-party installation-scope package directory is excluded from linked interception, while an installation-scope package unpacked under a `node_modules` directory is admitted to it: the peers it declares belong to the running installation, so they resolve from the installation's own modules rather than from a copy beside the package. That admission applies only when the installation also runs first-party directories outside `node_modules` (a source checkout, judged by a link's target rather than its path); an installation whose every package is unpacked keeps native lookup throughout, because both lookups already reach the same copy and probing a packaged filesystem's absent candidates raises errors instead of reporting absence. Overlapping roots do not change the importer's lookup order ([rule](../../../.agents/notes/implemented/architecture/2026-09-19-profile-resolution-lookup-order.md)).
