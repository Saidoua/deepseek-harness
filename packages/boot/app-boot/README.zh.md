...
- **链接目录。** profile 链接到树外目录时，其下的 importer 参与逐层 peer 查询，即使目标没有自身的 `package.json`。在每个 `D/node_modules` 位置，当前 `D/package.json` 的 peer 包名若存在于运行时表，就使用运行时包；其他包名查询物理候选。更近的物理包先于后续 peer 声明，peer 位置无需物理 `node_modules`。第一方 installation 作用域包目录不参与 linked 拦截；解包在 `node_modules` 目录下的 installation 作用域包则参与：它声明的 peer 属于当前运行的安装，因此从该安装自身的模块解析，而不是从包旁边的副本解析。该接纳只在安装同时运行 `node_modules` 之外的第一方目录时生效（源码 checkout，按链接目标而非链接路径判定）；若安装的每个包都是解包形态，则全程保持原生查询——两种查询本就到达同一副本，而在打包文件系统上探测不存在的候选会抛出错误而非报告不存在。重叠 root 不改变 importer 的查询顺序（[规则](../../../.agents/notes/implemented/architecture/2026-09-19-profile-resolution-lookup-order.zh.md)）。
