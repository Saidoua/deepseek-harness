...
`text-direction.css` owns the only direction selector in the client. The locale plugin publishes the active language's reading order as `data-dsh-text-direction` on the root and never writes `html[dir]`, so the application frame keeps left-to-right placement in every language; this sheet gives elements a component marks with `data-dsh-text-zone` the language's direction, bidi isolation, and reading-edge text alignment. It also publishes `--dsh-text-dir`, the inline-axis sign a glyph drawn with a transform multiplies its horizontal component by, because such a glyph cannot mirror through logical properties. Its spec rejects any other package stylesheet that selects `[dir=…]` or declares `direction`/`unicode-bidi`, and holds the stylesheets that style zone content to logical inline-axis properties.
