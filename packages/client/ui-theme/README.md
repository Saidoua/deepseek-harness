...
`src/styles/` holds nine sheets imported in order by ui-theme's dynamic client entry: `base.css`, `corner-shape.css`, `design-platform.css`, `focus.css`, `onboarding.css`, `scrollbar.css`, `gradient-shadow-text.css`, `shiki.css`, and `text-direction.css`. The client bundle compiles and injects them as plugin-owned global styles, so unload and HMR remove them with ui-theme. `scrollbar.css` consumes the `--dsw-alias-scrollbar-*` tokens and must follow `design-platform.css`, which declares them. Status marks use their own semantic state tokens. `design-platform.css` also owns the code-diff fill aliases and their static alpha palette entries, plus the `--dsw-alias-file-diff-*` code, gutter, and marker palette for file comparisons; `shiki.css` owns syntax colors.
...
`text-direction.css` owns every alignment decision the active language makes. The locale plugin publishes the language's reading order as `data-dsh-text-direction` on the root and never writes `html[dir]`; this sheet right-aligns the text inside elements a component marks with `data-dsh-text-zone` while Arabic is active. It deliberately never sets the `direction` property, which would reverse flex and grid children and move the settings navigation, the row controls, and every icon beside a label — a bilingual reader meets one layout in every language. Content pinned `dir="ltr"` and text marked `data-dsh-text-auto` opt back out, the second following its own words. Its spec pins the alignment-only rule, rejects the `direction` property outright, and rejects any other package stylesheet that selects `[dir=…]` or declares `direction`/`unicode-bidi`.
