...
describe('pressure measurement and retention', () => {
  ...
  it('does nothing below threshold and compacts a priced head above threshold', async () => {
    ...
    const session = conversation(4)
    // Automatic compaction spares the seed user turn: the span opens at the
    // first assistant node and the seed keeps surface position 0.
    const [seed, firstAssistant] = session.surface.nodes
    const result = await compactIfNeeded(compact, session)
    expect(result).not.toBeNull()
    expect(result?.shadowedSeqs[0]).toBe(firstAssistant)
    expect(session.surface.nodes[0]).toBe(seed)
  ...
  it('starts the range after a system head and keeps that node at surface position 0', async () => {
    ...
    const [head, firstUser, firstAssistant] = session.surface.nodes
    ...
    // Automatic compaction also spares the seed user turn, so the span opens
    // one node later and both head nodes keep their surface positions.
    const result = await compactIfNeeded(compact, session)
    expect(result).not.toBeNull()
    expect(result?.shadowedSeqs).not.toContain(head)
    expect(result?.shadowedSeqs).not.toContain(firstUser)
    expect(result?.shadowedSeqs[0]).toBe(firstAssistant)
    expect(session.surface.nodes[0]).toBe(head)
    expect(session.surface.nodes[1]).toBe(firstUser)
...
describe('seed user turn sparing (automatic compaction)', () => {
  ...
  it('declines an automatic span that holds only the previous checkpoint', () => {
    ...
    session.append('user/message', createUserMessage({
      ...
      surfaceOp: { op: 'replace', startSeq: compacted.seq, endSeq: compacted.seq },
