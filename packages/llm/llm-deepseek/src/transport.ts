...
export function providerError(raw: unknown, status: number | undefined, headers?: Headers): LlmError {
  ...
  let code: string
  // 401 is unauthenticated whatever the body says; a 403 naming an exhausted quota is QUOTA, not a bad key.
  if (status === 401 || type === 'authentication_error') code = 'AUTH'
  else if (isQuotaExceededError(detail) || status === 402) code = 'QUOTA'
  else if (status === 403 || type === 'permission_error') code = 'AUTH'
