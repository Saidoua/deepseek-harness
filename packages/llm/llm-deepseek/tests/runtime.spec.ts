...
describe('DeepSeekAdapter against a mock server', () => {
  ...
  it('reports an exhausted balance behind HTTP 403 as quota, not a bad key', () => {
    expect(providerError({ error: { code: 'insufficient_quota', message: 'Free quota exhausted.' } }, 403).code)
      .toBe(QUOTA_EXCEEDED_CODE)
    expect(providerError({ error: { type: 'insufficient_quota' } }, 403).code).toBe(QUOTA_EXCEEDED_CODE)
    expect(providerError({ error: { message: 'forbidden' } }, 403).code).toBe('AUTH')
    expect(providerError({}, 403).code).toBe('AUTH')
    // RFC 9110: 401 is unauthenticated, so it stays AUTH regardless of body wording.
    expect(providerError({ error: { code: 'insufficient_quota' } }, 401).code).toBe('AUTH')
  })
