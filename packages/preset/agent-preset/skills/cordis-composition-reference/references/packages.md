...
| `@deepseek-ai/dsh-client-locale-ar` | no | Arabic language pack: the ar language definition and its namespace dictionaries |
...
| `@deepseek-ai/dsh-compaction-prefix-slide` | no | Drop-in compaction-basic replacement: evicts the middle without any summarizer call (prefix sliding + structured state) |
...
| `@deepseek-ai/dsh-session-rules` | yes | Session-pinned standing rules: durable pins re-injected verbatim after compaction |
...
| `@deepseek-ai/dsh-skill-stats` | no | Session projection of skill invocations: usage and error counts per skill, from the durable log |
...
| Package | Config | Description |
|---|---|---|
| `@deepseek-ai/dsh-tool-state` | yes | Model-facing state_write tool: a typed mutable working state re-injected as one replacing snapshot |
